    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-body">
                <div class="row mb-3">
                    <div class="col-md-12">
                        <form action="{{ url('/siswa/nilai') }}" method="GET" class="row g-3">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                <div class="btn-group">
                                    ...
                                    <a href="{{ url('/siswa/nilai') }}" class="btn btn-secondary">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li><a class="dropdown-item" href="{{ url('/siswa/nilai/export/csv' . (request()->getQueryString() ? '?' . request()->getQueryString() : '')) }}">Export CSV</a></li>
                                        <li><a class="dropdown-item" href="{{ url('/siswa/nilai/export/json' . (request()->getQueryString() ? '?' . request()->getQueryString() : '')) }}">Export JSON</a></li>
                                    </ul>
                                </div>
                            </div>
                        </form>
                            ...
                            @if(count($nilai) > 0)
                                @foreach($nilai as $index => $n)
                                    <tr>
                                        ...
                                        <td>
                                            @if($n->pengumpulanTugas->status == 'submitted')
                                                <span class="badge bg-success">Sudah Mengumpulkan</span>
                                            @elseif($n->pengumpulanTugas->status == 'late')
                                                <span class="badge bg-warning">Terlambat</span>
                                            @endif
                                        </td>
                                        <td>{{ date('d M Y H:i', strtotime($n->pengumpulanTugas->waktu_pengumpulan)) }}</td>
                                        <td><span class="badge bg-primary">{{ $n->skor }}</span></td>
                                        <td>
                                            <a href="{{ url('/siswa/nilai/show/' . $n->id) }}" class="btn btn-info btn-sm">
                                                <i class="ph ph-eye"></i> Detail
                                            </a>
                                            <a href="{{ url('/siswa/tugas/show/' . $n->pengumpulanTugas->tugas_id) }}" class="btn btn-primary btn-sm">
