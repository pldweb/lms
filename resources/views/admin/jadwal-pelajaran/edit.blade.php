    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="/admin/jadwal-pelajaran/{{ $jadwal->id }}" id="jadwalForm">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-9">
                                <input type="time" class="form-control" id="jam_mulai" 
                                       name="jam_mulai" value="{{ old('jam_mulai', $jadwal->jam_mulai) }}" required>
                        ...
                        <div class="row">
                            <div class="col-sm-9 offset-sm-3">
                                ...
                                <a href="/admin/jadwal-pelajaran/{{ $jadwal->id }}" class="btn btn-outline-info">
        ...
        <div class="col-lg-4">
                ...
                <div class="card-body">
                    <div class="current-schedule-info">
                        <div class="row mb-2">
                            ...
                            <div class="col-8">{{ $jadwal->kelas_jenjang }} {{ $jadwal->kelas_tingkat }} {{ $jadwal->kelas_nama }}</div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-4"><strong>Mapel:</strong></div>
                            <div class="col-8">{{ $jadwal->mata_pelajaran_nama }}</div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-4"><strong>Guru:</strong></div>
                            <div class="col-8">{{ $jadwal->guru_nama }}</div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-4"><strong>Hari:</strong></div>
                            <div class="col-8">{{ $jadwal->hari }}</div>
                        </div>
                        <div class="row mb-2">
                            <div class="col-4"><strong>Waktu:</strong></div>
                            <div class="col-8">{{ $jadwal->jam_mulai }} - {{ $jadwal->jam_selesai }}</div>
                        </div>
                        @if($jadwal->ruangan)
                        <div class="row mb-2">
                            <div class="col-4"><strong>Ruangan:</strong></div>
                            <div class="col-8">{{ $jadwal->ruangan }}</div>
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="card mb-3">
                ...
                <div class="card-body" id="conflictCheck">
                    <div class="text-muted text-center py-4">
                        ...
                        <p>Ubah data untuk mengecek bentrok jadwal</p>
                    </div>
                </div>
            </div>
            ...
            <div class="card mb-3">
                ...
                <div class="card-body" id="daySchedule">
                    <div class="text-muted text-center py-4">
                        <i class="fas fa-calendar-day fa-3x mb-3"></i>
                        <p>Pilih hari untuk melihat jadwal lainnya</p>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <ul class="list-unstyled mb-0">
                        <li class="mb-2">
                            ...
                            Pastikan perubahan tidak menimbulkan bentrok baru
                        </li>
                        <li class="mb-2">
                            <i class="fas fa-check text-success me-2"></i>
                            Koordinasikan dengan guru jika mengubah jadwal
                        </li>
                        <li class="mb-2">
                            <i class="fas fa-check text-success me-2"></i>
                            Informasikan perubahan kepada siswa
                        </li>
                        <li>
                            <i class="fas fa-check text-success me-2"></i>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function checkConflicts() {
        ...
        formData.append('exclude_id', currentJadwalId); // Exclude current schedule from conflict check
        ...
        if (data.conflicts && data.conflicts.length > 0) {
            ...
            html = `
                <div class="no-conflict">
                    ...
                    <small>Perubahan dapat disimpan dengan aman</small>
                </div>
            `;
        }
    ...
    function loadDaySchedule() {
        ...
        .then(response => response.json())
        .then(schedules => {
            displayDaySchedule(schedules, hari);
        })
        .catch(error => {
            document.getElementById('daySchedule').innerHTML = `
    ...
    function displayDaySchedule(schedules, hari) {
        ...
        if (schedules.length === 0) {
            ...
            schedules.forEach(schedule => {
                const isCurrent = schedule.id == currentJadwalId;
                const itemClass = isCurrent ? 'current-schedule-item' : 'schedule-item';
                const timeClass = isCurrent ? 'current-schedule-time' : 'schedule-time';
                
                html += `
                    <div class="${itemClass}">
                        <div class="${timeClass}">
                            ${schedule.jam_mulai} - ${schedule.jam_selesai}
                            ${isCurrent ? '<span class="badge bg-warning ms-2">Saat ini</span>' : ''}
                        </div>
